...
- **ID 포맷**: `<CAT>-<NNNN>` (4자리 zero-pad). 파일명 = `<ID>-<kebab-slug>.md`. ID는 영구 고정하고 슬러그/내용만 바꾼다.
- **cross-link**은 본문에 ID 텍스트(`DP-0002`, `QA-0004`)로 적어 grep 역참조가 되게 한다.
...
- **QA canonical 번호 = pptx 슬라이드 13 기준**으로 통일한다.
