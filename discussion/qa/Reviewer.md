...
- **★ 등급 척도(rubric) 경계 현실성**(있으면): ★☆☆/★★☆/★★★ 급간 경계가 필드 현실에 맞는가 — 하한이 비현실적으로 높아 **★★★가 사문화**되거나, 너무 보수적이라 **★☆☆가 사문화**되지 않는가? 2-index가 main 급간 축 + 조건으로 바르게 분해됐나? (이 지적은 Council [§9 등급 척도 캘리브레이션](Council.md)으로 처리 — 경계의 **필드 벤치마크 근거는 렌즈 1·3이 보강**한다.)
...
| **Realistic** | agentic·on-device 현실에서 달성 가능한가? | LLM 비결정·rate limit·토큰 경제·외부 의존 반영 · (등급 척도 있으면) 급간 경계가 필드 현실 — ★★★·★☆☆ 비사문화 |
...
- cross-link은 ID 텍스트(`QA-03`, `DP-01`, `QAS-05`)로 적어 grep 역참조되게.
...
0. **직전 라운드 반영 보고서 받기**(`round-NN/applier/report.md`가 있으면 — 1라운드면 생략). 각 지적의 disposition을 확인하고 이번 라운드에 반영한다:
   - **[반영]** 항목 → 원본이 실제로 그 지적을 해소했는지 **재평가**(verdict가 개선됐나? 새 결함이 생겼나?). report에 verdict 변화 기록.
   - **[발표 서사]/[이월]** 항목 → 아직 안 닫혔으니 **이번 라운드에서 다시 판정**(서사로 미룬 검증이 여전히 유효한 설계인지).
   - **[거부]** 항목 → 사유가 타당한지 점검, 필요하면 재반박 또는 수용.
   - 보고서 §3 "다음 Reviewer가 다시 볼 것"이 곧 **이번 라운드의 우선 점검 목록**이다.
