...
- **번호 주의**: DP는 **2자리** ID(`DP-02`)로 통일한다(QA·QAS와 동일 포맷). DP 번호는 발표 우선순위가 아니므로 ID는 **고정**하고 슬러그/내용만 바꾼다(재번호 안 함). 신규 결정 채택 시 다음 빈 번호 부여. (구 4자리 `DP-0001/0002`는 2026-06-27 삭제 — DP-0002→DP-01 승계, 구 DP-0001은 재귀속 보류 OI-12. 잔여 4자리 `DP-0003/0004/0005`의 2자리 전환은 별도 마이그레이션.)
