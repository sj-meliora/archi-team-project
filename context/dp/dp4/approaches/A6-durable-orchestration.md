...
> 근거 리서치: R-03 | drives: QA-06(주), QA-10, QA-08, QA-01 | realizes: FR-0001
...
## QA별 장점 / 단점
- **[Reliability-Workflow QA-06] ★★★ (주 강점)**: durable replay로 crash 후 무손실 복구, 실패 노드만 자동 retry/보상 → 타 Workflow 무영향(중단≤1% 직접 겨냥). immutable event log로 SPOF 완화.
- **[Maintainability QA-10] ★★★**: workflow를 코드/선언으로 명시 → 흐름 가시성↑, 노드 추가·변경·재정렬이 명시적이라 Change Impact 추적 용이.
- **[Performance-E2E QA-08] ★★☆**: ⚠️ event log 영속화·엔진 hop 오버헤드. 단 장기 실행 timeout 부재로 대형 컴파일에 안정. 전달 오버헤드 ≤5% 검증 필요.
...
## mini-ATAM
- **SP**: 이벤트 스토어 영속화 빈도가 QA-08(오버헤드≤5%)에 민감. 오케스트레이터 샤딩 여부가 QA-01에 민감.
