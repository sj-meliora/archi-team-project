# DP-06 자율 Agent를 위한 지식베이스 설계 (Knowledge Base for Autonomous Agents) — RAG vs LLM Wiki
...
> 요지: **실행하는 Agent는 명령이면 충분하지만, 참여하는 Agent는 지식이 있어야 한다** — 지식베이스는 능동 참여(AI-DLC)의 성립 조건이다. 그리고 지금 그 지식베이스는 **비어 있다**(기존 Confluence·Jira 연동 RAG도, LLM 위키도 없음). 어떻게 채울 것인가 — 다음 장에서 구조(슬라이드 2·3)와 방식(슬라이드 4)을 결정한다.

### 슬라이드 2 — 공통 조회 계층 구조: 하나의 인덱스, 여러 지식 소스

**메시지**: LLM 위키든 기존 Confluence·Jira·채팅이든, 모든 지식 소스는 **하나의 조회 계층(단일 인덱스) 아래에 federation으로 붙는다.** 본 DP의 결정(슬라이드 4)은 이 구조에서 "지식을 어떤 방식으로 채우나" — 인덱싱 경로의 선택이다.
...
- **인덱싱 경로가 대안을 가른다**: 1안 RAG는 기존 소스를 인덱싱(기본: raw 직결(점선) / 강화: **증류 시딩** 경유), 2안 LLM 위키는 운영 이벤트를 증류 축적하되 **judge 검증**을 거쳐 등재 — 대안별 상세 흐름은 슬라이드 3(구조도), 비교·결정은 슬라이드 4.

### 슬라이드 3 — 대안 구조도: 질의와 지식은 어떻게 흐르는가

**메시지**: 두 대안은 "Agent가 무엇을 조회하느냐"가 아니라 **지식이 어디서 와서 어떤 경로로 인덱스에 들어가느냐**가 다르다 — 1안은 기존 지식 공간(Confluence·Jira·Messenger·빌드 로그)이 배선돼 있고, 2안은 운영 루프에서만 지식이 생긴다(콜드스타트가 구조에서 보인다).

| 1안 RAG | 2안 LLM 위키 |
|---|---|
| ![1안 RAG 구조도](diagrams/1an-rag.svg) | ![2안 LLM 위키 구조도](diagrams/2an-llm-wiki.svg) |

**읽는 포인트**
- **1안 RAG**: ⓪ 사전에 기존 소스를 인제스천(기본: raw 직결(점선·garbage-in 위험) / 강화: **증류 시딩** 경유) → ① Agent 질의(C-03 게이트 경유 — **소스 직접 접근 없음**) → ② 하이브리드 검색·RRF 융합 → ③ top-k 근거 반환 → ④ 근거 기반 분석·해결.
- **2안 LLM 위키**: ① 운영 이벤트(이슈 close·config 변경)를 증류해 문서 초안 → ② **LLM-as-a-judge 검증**(κ≥0.8, 불합격 반려) → ③ 통과분만 위키 등재 → ④ Agent 질의(목차·검색) → ⑤ 문서 근거 반환 → ⑥ 처리 결과가 새 이벤트로(선순환). **기존 지식 공간과는 연결이 없다**(✕ 표시) — 콜드스타트 약점이 도식에서 그대로 드러남.

### 슬라이드 4 — RAG vs LLM 위키: 비교·결정 (+ 각자의 강화 tactic)

> 대안별 column = (a) 도안 + (b) 설명 + (c) 별점(행=ASR·KB-DQ). 증류 시딩의 필드 실증 = Cerebras Knowledge(A7·Appendix 슬라이드).

| | **1안 · RAG (기존 지식 검색)** | **2안 · LLM 위키 (Agent가 지식을 문서로 축적)** |
|---|---|---|
| **(a) 도안** | [`diagrams/1an-rag.svg`](diagrams/1an-rag.svg) (슬라이드 3) | [`diagrams/2an-llm-wiki.svg`](diagrams/2an-llm-wiki.svg) (슬라이드 3) |
...
본편에서 빠진 **필드 실증 상세**를 발표 Appendix 1장으로 유지: 파이프라인(소스 → LLM 증류 → 단일 임베딩 스토어 → 하이브리드 검색 → RRF·rerank → age decay → 인용 답변) + 3가지 교훈 매핑(raw를 임베딩하지 않는다 → 슬라이드 4 1안 강화 tactic(증류 시딩)의 원형 / 순수 시맨틱 부족 → 하이브리드 필수 / age decay → R-4 완화). 전문·출처는 A7.
