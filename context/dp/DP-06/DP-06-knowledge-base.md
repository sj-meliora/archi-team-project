...
> category: DP | status: 초안 v4.2 (2026-08-13 2안 조회 경로 2단화 — 팀 검토 대기) | source: 신규 발굴 (자율 이슈처리 서사의 지식 공급 공백) + 필드 레퍼런스(Cerebras Knowledge·Wikipedia 문서 보호 정책·Blackboard 패턴) | updated: 2026-08-13
> drives: QA-07(Correctness)↑, QA-05(Efficiency) | **v4.1 보조 접점(거버넌스 축 — §A3′)**: QA-01(갱신 처리량 확장)·QA-03(HITL 게이트 원칙)·QA-04(결정 재구성) | 비-ASR 참고: QA-08·11·12·13 | 평가 보강: **KB-DQ**(지식 데이터 품질 — ISO/IEC 25012 앵커: 커버리지·신선도·축적 속도·**정합성(v4 신설)**, §지식 품질 평가 기준)
> realizes: FR-0003(Regression 관리의 지식 축), FR-0004(이슈처리 근거 추적) | constrained-by: C-03(지식 접근도 권한 게이트 통과)
> **재프레이밍 이력**: v1(2026-08-10) "RAG vs LLM Wiki" 형태 비교 → **v2(같은 날 디스커션)** 인덱싱·조회 계층은 공통 전제임을 확인, 결정 축을 부트스트랩 전략(Backfill/Forward/Distill-seeded)으로 전환 → **v3(같은 날 발표 용어 정비)** 대안을 1안 RAG / 2안 LLM 위키로 단순화, 증류 시딩·LLM-as-a-judge를 강화 tactic으로 재배치 → **v3.1(같은 날)** KB-DQ 신설(ISO/IEC 25012 앵커) → **v3.2(같은 날)** 설계명·배경 논지("실행을 넘어 참여로") 확정 → **v4(2026-08-13 거버넌스 재프레이밍)** 팀 방향 전환: **LLM 위키를 기반(base)으로 채택** — v3 A4 수렴 노트("증류 시딩의 산출과 위키 문서는 동형") 채택으로 형태 택일 종료. 위키는 기존 소스(**IMS(Jira)·Confluence·Slack**) **증류 md 문서로 구축**(콜드스타트를 증류 시딩으로 닫음) + 운영 축적. 결정 축 전환: "무엇으로 채우나" → **"위키 갱신을 어떻게 거버넌스하나"** — 새 대안 = **1안 지식 등급별 차등 게이팅**(정합성 엄격 지식=사람(시니어·아키텍트) PR 게이트 / 팁·현황=LLM-as-a-judge auto-merge) vs **2안 엄격 정본 + 런타임 공유 계층**(정본은 전량 사람 PR 게이트, Agent 생성 최신 정보는 위키 밖 런타임 계층에서 공유). KB-DQ-4(Consistency) 신설 → **v4.1(같은 날 전 QA 스크리닝)** repo 정의 QA 12종 전수 판정(§A3′) — v3.1의 "물리는 ASR은 QA-07·QA-05뿐"은 '무엇으로 채우나' 축 기준이었고, **거버넌스 축에서는 QA-01·QA-03·QA-04가 추가로 변별**됨을 확인(+비-ASR 참고 QA-08·11·12·13) → **v4.2(같은 날 2안 조회 경로 2단화)** 런타임 note의 전량 컨텍스트 주입을 기각(QA-05 캡 잠식·lost-in-the-middle 재발·관련성 판단 주체 전도) — **다이제스트 push**(한 줄 목차 — 존재 알림, unknown unknowns 해소) + **선택 pull**(get_note/query_notes — 필요분은 Agent가 판단해 조회) 2단으로 정정. 명시 조회만 citations 집계(승격 신호 품질 조건). 미결정 트래킹: open-issues OI-13.
...
| **런타임 공유 계층 (runtime sharing layer)** | 위키(영속 정본) **밖**에서 Agent들이 작업 중 생성한 최신 정보(발견·현황·임시 해결책)를 **게이트 없이 즉시 게시**하는 비영속 계층 — 고전 **Blackboard 패턴**의 적용. TTL·출처 표식 부착. 수신은 **2단(v4.2)**: **다이제스트 push**(scope 매칭 note의 한 줄 목차만 자동 주입 — 존재 알림) + **선택 pull**(전문은 Agent가 get_note/query_notes로 필요분만 조회 — 전량 주입 기각) |
...
- **2안 정본+런타임**: 위키는 **전량 사람 PR 게이트**(등재분 100% 사람 검증 — 순도 최상). ① Agent가 작업 중 발견·현황을 **런타임 공유 계층에 즉시 게시**(게이트 없음) → ② 다른 Agent는 작업 시작 시 **다이제스트(scope 매칭 한 줄 목차)만 주입**받고, 필요분만 **선택 조회**(get_note — TTL·provenance 표식 부착, 미검증 근거로는 비가역 액션 금지) → ③ 가치 있는 항목은 **승격 큐 → 증류 → 사람 게이트 → 정본 등재** → ④ 미승격분은 TTL 만료로 소멸. 위험 표시: **사람 게이트 병목** — 승격이 시니어 리뷰 대역폭에 종속, 큐 적체 시 지식이 만료로 유실된다.
...
> ★ 앵커 — **[ASR] 행**: Correctness = golden 정답률 [93,99]=★★★(QA-07 — 정본/운영 분리는 두 지식군의 오염 경로·파급이 달라서) · Efficiency = 작업당 신규 토큰 ≤4k=★★★(QA-05, 캐시 분리집계). **[KB-DQ] 행**: §지식 품질 평가 기준(ISO/IEC 25012 앵커). **◯** = 조건부 — 1안 정본 ★★★◯는 오분류 표본 감사 규율 전제, 1안 운영 ★★★◯는 judge κ≥0.8 선검증 조건(QA-07과 동일). 강화 tactic 비용(judge 판정 토큰·시니어 리뷰 공수·승격 큐 운영)은 비-ASR(QA-13) → 교환점 TP-2로 관리. **repo QA 12종 전수 판정(v4.1 — QA-01·03·04 추가 변별 + 비-ASR 참고)은 A3′** — 본편 표는 발표용 핵심 행만 유지.
...
**2안. 엄격 정본 + 런타임 공유 (Canonical + Runtime Sharing)** — 구조: 위키는 전량 사람 PR 게이팅(순도 100%), Agent 생성 최신 정보는 위키 밖 런타임 공유 계층(Blackboard 패턴)에 무게이트 즉시 게시(TTL·provenance 표식, 미검증 근거 비가역 액션 금지 = C-03), 수신은 **2단 조회**(다이제스트 push + get_note/query_notes 선택 pull — 전량 주입 기각, v4.2), 승격 큐→증류→사람 게이트→정본. tactic: 승격 큐 SLA·리뷰 당번제, TTL·age decay, provenance 태깅, 2단 조회. 장점 [KB-DQ-4·Correctness 정본] 구조적 순도 보장 — 오염이 영속층 진입 불가 / [KB-DQ-2] 런타임 즉시 공유. 단점 [KB-DQ-1·3] 영속 축적이 사람 대역폭 종속(병목) — 큐 적체 시 TTL 유실 / [Correctness 운영] 런타임 무검증 전파 / [Efficiency] 이중 조회·캐시 비친화.
...
### A3′. 전 QA 스크리닝 (v4.1 — repo 정의 QA 12종 전수 판정)

> **왜 전수 판정인가**: v3.1은 "기존 ASR 중 실제로 물리는 건 QA-07·QA-05뿐"이라 했으나, 그건 **'무엇으로 채우나' 축** 기준이었다. 결정 축이 거버넌스로 바뀌자 물리는 QA가 달라졌다 — 갱신 게이트는 **확장성(처리량)·통제성(HITL)·관측성(재구성)** 문제이기도 하다. 아래는 repo QA 전체(QA-01~13, QA-06은 C-03 이관·공석)의 접점·변별 전수 판정. **비-ASR 행은 참고 별점** — DP 생성 동인이 아니며 과최적화 금지([`asr.md`](../../asr.md)). 이중 계상 방지: 간접 영향은 매개 QA 행에 흡수하고 별점을 주지 않는다.

**[ASR] 판정** (우선순위 순 — [`asr.md`](../../asr.md)):

| QA (ASR 우선순위) | 판정 | 1안 차등 게이팅 | 2안 정본+런타임 | 근거 |
|---|---|:---:|:---:|---|
| **QA-01 Scalability** (1) | **변별 (v4.1 신규)** | **★★★** | ★☆☆ | 워크로드(140+ 모델·이슈량)가 늘면 **지식 갱신 이벤트도 비례 증가**한다. 1안 judge auto-merge는 병렬 확장 가능, **2안 사람 게이트는 고정 대역폭이라 갱신 처리량이 스케일 불가** — 부하 증가가 곧 승격 큐 적체(R-2 악화). 단 judge 호출도 공유 TPM/RPM 소비 — QA-08 쿼터 격리로 관리(백그라운드 배치라 파이프라인 경합은 낮음) |
| **QA-02 Availability** (2) | 비변별 (N) | — | — | KB는 soft-dependency — 조회 실패 시 grounding 없이 degrade·재시도(양안 동일, 조회 계층 공통). 2안 런타임 계층은 명시적 비영속이라 고가용 요구 자체가 없음(다운 = 최신 공유 일시 중단, 정본 조회는 무영향) |
| **QA-03 Controllability** (3) | **변별 (v4.1 신규)** | ★★☆◯ | **★★★** | **위키 갱신도 액션이다.** 2안은 "영속 쓰기 = 전량 HITL"로 QA-03의 고위험 HITL 게이트 원칙에 구조적으로 정합하고, provenance의 "미검증 근거 비가역 액션 금지"는 QA-03 admission 게이트 메커니즘의 재사용. 1안 auto-merge는 **사람 승인 없는 영속 쓰기의 허용** — 대상을 저위험(운영 티어)으로 한정하고 judge 에스컬레이션 룰로 보상(◯ = 그 규율이 조건). 갱신 폭주 rate cap(QA-03 runaway cap의 지식판)은 양안 공통 필요 |
| **QA-04 Observability** (4) | **변별 (v4.1 신규)** | **★★★** | ★★☆ | "왜 이 판단을 했나"의 재구성에는 **그때 참조한 지식**이 포함된다. 1안 = 단일 위키 + git 이력 → 참조 시점의 문서 버전을 커밋 단위로 완전 재구성. 2안 런타임 note는 **TTL 소멸 후 사후 재구성에 공백** — 감사용 note 아카이브 로그(소멸 후에도 보존) 보완 필요(= 추가 컴포넌트) |
| **QA-05 Efficiency** (5) | 변별 (기존 v4) | **★★★** | ★★☆ | 1안 단일 위키·안정 문서 = prompt cache 친화. 2안 정본+런타임 이중 조회, 런타임은 갱신이 잦아 캐시 비친화 (A3 표와 동일). v4.2 2단 조회로 주입분은 다이제스트 상한 내 관리 — 캡 잠식 경로는 닫힘, 이중 조회·캐시 판정은 유지 |
| **QA-07 Correctness** (6) | 변별 (기존 v4) | 정본 ★★★◯ / 운영 ★★★◯ | 정본 **★★★** / 운영 ★★☆ | A3 표와 동일 — 정본은 2안이 구조적 우위, 운영·최신 지식은 1안이 검증 등재로 우위 |

**[비-ASR] 참고 판정** (DP 생성 동인 아님 — 별점은 보조):

| QA | 판정 | 1안 | 2안 | 근거 |
|---|---|:---:|:---:|---|
| **QA-08 Reliability(WF 격리)** | 변별 | ★★☆ | **★★★** | **KB는 전형적 공유 상태** — 오염 지식의 전파가 곧 blast-radius 문제(QA-08 캐시 오염 KPI와 동형). 1안 운영 티어 오염은 전 Agent에 영속 전파(반경 大 — judge·감사가 방어선), 2안은 TTL·scope로 국소화 + 정본 격리(반경 小) |
| **QA-09 Performance(Agent 시간)** | 비변별 | — | — | first-pass 성공률은 지식 품질을 매개로 한 간접 영향 — **QA-07 행에 흡수**(이중 계상 금지) |
| **QA-10 Performance(E2E)** | 비변별 | — | — | 조회 latency·처리량은 공통 조회 계층(BL-4)의 속성 — 대안 변별 아님 |
| **QA-11 Reliability(결정 일관성)** | 약변별 | **★★★** | ★★☆ | 지식 컨텍스트의 안정성이 유효-결정 안정성의 입력이다. 1안 = 안정 영속 문서(버전 고정 가능 — 재실행 시 동일 근거 재현). 2안 = 런타임 note의 존재 여부가 시점마다 달라 유사 입력 재실행의 변동 요인(단 결과가 유효하면 QA-11 정의상 허용 — 유효-결정률로 관리) |
| **QA-12 Maintainability** | 변별 | **★★★** | ★★☆ | 1안 신규 유지보수 표면 = 티어 경계 룰 + judge 프롬프트(위키·QA-07 하네스 재사용 — CIS 소폭). 2안 = note 스키마·TTL 정책·승격 워커·admission 인용 검사·아카이브 로그 등 **신규 컴포넌트 一式** — CIS·운영 표면 증가 |
| **QA-13 Cost-economy** | 변별 | **★★★** | ★★☆ | 1안 = judge 판정 토큰(갱신 건당·소액) + 표본 감사 공수. 2안 = 증류 토큰 + **시니어 리뷰 공수가 지배 항** — "수작업 대비 절감"이 top-line인 QA-13에 사람 시간을 시스템 운영 비용으로 재도입하는 역행(승격 큐 SLA가 곧 사람 공수 약정) |

**합산 관찰 (전수 스크리닝 결론)**
- **1안 우세**: QA-01(확장)·QA-04(재구성)·QA-05(캐시)·QA-07 운영(검증 등재) + 비-ASR QA-11·12·13 — "축적·운영이 사람과 시스템에 싸게 먹힌다"로 수렴.
- **2안 우세**: QA-03(HITL 원칙)·QA-07 정본(구조적 순도) + 비-ASR QA-08(blast-radius) — "통제·격리가 구조로 보장된다"로 수렴.
- **비변별**: QA-02·09·10 (+ QA-06은 C-03 이관 — 양안 공통 pass/fail 제약, NR-1).
- **권고(1안) 유지** — ASR 우선순위 1위(QA-01)와 4·5위(QA-04·05)가 1안 편, 2안 편은 3위(QA-03) 하나. 단 **QA-03 관점의 보상 통제(judge 에스컬레이션 룰·오분류 표본 감사·갱신 rate cap)를 이행 조건으로 명시** — 이것이 1안 ★◯들의 해제 조건과 동일 집합이라 추가 비용이 아니다.
...
- **2안 + 2단 조회 (v4.2 — 다이제스트 push + 선택 pull)**: 작업 시작 시 하네스는 scope 매칭 note의 **한 줄 다이제스트(id·scope·claim 요지·TTL 잔여·citations, 상한 N건 — recency·citations 정렬)만 주입**하고, 전문은 Agent가 **get_note(id)로 필요분만 조회**(작업 중 능동 검색은 query_notes — 조회 계층 질의와 동일 패턴, note는 federation 소스로 일반화). **전량 주입 기각 사유**: ⑴ QA-05 신규 토큰 캡(≤6k/8k)을 note 더미가 잠식 + 매 세션 조합 변동으로 캐시 파괴 ⑵ lost-in-the-middle 재발(A1의 "다 넣기" 기각 논거가 런타임 계층에서 재현) ⑶ 관련성 판단 주체는 작업 맥락을 아는 Agent지 하네스가 아님. **완전 무주입(다이제스트 0)도 기각**: Agent가 note의 존재 자체를 몰라 조회를 시도하지 않음(unknown unknowns) — 공유 계층의 존재 이유 상실. 부수 효과: **명시 get_note만 citations로 집계** → 승격 신호(④ 워커 입력)의 품질 조건("컨텍스트에 있었음"과 "실제로 참조함"의 구분). 잔존 리스크: 목차만 보고 필요 note를 안 가져가는 판단 실수 — 다이제스트 한 줄의 증류 품질(post_note 규율)로 완화(SP-6).
...
### A5. ATAM 분석
**민감점** — SP-1(티어 경계 정의·분류 품질 → 정본 정합성, 1안) · SP-2(judge κ → 운영 티어 품질, 1안 / 승격 사전 필터 품질, 2안) · SP-3(시니어 리뷰 대역폭 → 승격 큐 체류시간·축적 속도, 2안) · SP-4(TTL 길이 → 신선도↔유실·오염 노출시간의 양날, 2안) · SP-5(위키 문서 안정성 → 캐시 적중률 → QA-05, 양안) · SP-6(다이제스트 한 줄 증류 품질 → 선택 조회 누락률 → 공유 실효성, 2안 v4.2).
