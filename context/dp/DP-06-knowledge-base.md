...
> 요지: **실행하는 Agent는 명령이면 충분하지만, 참여하는 Agent는 지식이 있어야 한다** — 지식베이스는 능동 참여(AI-DLC)의 성립 조건이다. 그리고 지금 그 지식베이스는 **비어 있다**(기존 Confluence·Jira 연동 RAG도, LLM 위키도 없음). 어떻게 채울 것인가 — 다음 장에서 구조(슬라이드 2)와 방식(슬라이드 3)을 결정한다.
