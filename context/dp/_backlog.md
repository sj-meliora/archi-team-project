...
- **맥락**: 2026-08-10 DP-06 디스커션 — 인덱싱·조회 계층은 어느 안이든 공통 필수 인프라(변별 축 아님)로 확인 → DP-06은 채택 패턴(Cerebras Knowledge 레퍼런스: 단일 임베딩 스토어 + 하이브리드 검색 + RRF·rerank + age decay)만 전제로 깔고 상세 설계를 후속으로 분리. **2026-08-13 v4**: 위키 기반·증류 구축은 공통 전제로 확정 — 남은 상위 결정은 거버넌스(1안 차등 게이팅 vs 2안 정본+런타임 공유). 2안 채택 시 런타임 공유 계층의 인덱싱(short-lived 인덱스·TTL 반영)도 본 백로그 범위에 추가.
- **근거 tactic/pattern**: 하이브리드 검색(전문검색: 에러 코드·플래그명 정확 일치 + 임베딩: 패러프레이즈), 희소어 가중치, RRF 융합, cross-encoder rerank, age decay(staleness 랭킹 완화 + v4: 런타임 항목 자동 강등).
- **미검증 trade-off / 리스크**: 우리 규모에서 풀 하이브리드가 과한지(초기엔 구조적 인덱스+전문검색으로 충분할 수 있음 — 증류 시딩 규모에 종속), 인덱스 유지 비용(QA-13).
- **상태**: 검토 대기 — DP-06(거버넌스) 확정 후 승격 여부 결정.
