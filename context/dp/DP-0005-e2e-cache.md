...
> drives: QA-0008(Performance-E2E), QA-0006(Reliability-Workflow), QA-0009(Reliability-Agent), QA-0010(Maintainability)
> realizes: FR-0002
...
- **SP-1**: 캐시 공유 범위가 Performance(QA-0008, E2E)와 Reliability-Workflow(QA-0006)에 동시 민감.
- **SP-2**: 캐시 공유가 Agent 판단 일관성(QA-0009, 재현율≥80%)에 민감.

### 교환점 (Tradeoff Points)
- **TP-1 (Performance/Agent일관성 ↔ Workflow독립성)**: 공유(2안)는 재사용·일관성↑·장애전파 위험↑, 로컬(1안)은 반대. 본 DP의 핵심 교환점.

### 위험 (Risks)
- **R-1**: 2안은 공유 캐시 오염이 다수 Workflow로 전파 → QA-0006(중단≤1%) 미달 위험.
- **R-2**: 1안은 재사용 불가로 QA-0008(E2E) 개선폭 제한.
