...
- **(보조·open-issue) ②-2 정밀 유효-결정률** — golden 대비 정답성까지 본 유효 비율 (NQA-B 의존)
  - 쉽게: "무효 아님"을 넘어 "정답인가"까지 보려면 NQA-B golden이 있어야 한다 → 미구축이라 **추적되는 open-issue로 강등**(닫힌 KPI 아님). *placeholder(숨은 빈틈)가 아니라 선언된 open-issue(추적되는 빈틈).*
...
| ②-2 정밀 유효-결정률 | **NQA-B(Correctness) golden 게이트**(교차 의존, open-issue 강등) | (open-issue) NQA-B 구축 전 미산출 — 닫힌 KPI 아님 |
...
### 2026-06-24 — round-01 contention 반영 (QA-09)
...
- **R2**: 유효-결정률 ②의 앵커(NQA-B golden)가 [생략] → ②는 값 낼 경로 없는 placeholder("일관+정확 닫힘"이 서류상뿐).
...
- **② 2단 분리(R2 부분수용)**: ②-1 대리 유효-결정률(정책 룰셋/제약 위반 검사, golden 불요 → **[반영]**) + ②-2 정밀(NQA-B 의존 → **open-issue 명시 강등**). placeholder를 추적되는 open-issue + 측정 가능한 게이트로 전환.
- **importance 게이트(R3 수용)**: 상향을 R1·R2 해소 **선결 조건**으로(미충족 시 L 유지).
- 짝 `QAS-09` Measure 동기화.

**남은 일**
- **②-2 ↔ NQA-B golden 교차 의존** — `open-issues.md` OI-7 트래킹. NQA-B 구축 시 닫힘.
