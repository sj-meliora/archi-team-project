...
- **신규 토큰 ≤ 6k(일반) / ≤ 8k(복합 추론 노드) AND 캐시 토큰 별도 집계** `[주 KPI · PoC 대상]` — 총 토큰 = 입력+출력(명시), 캐시 적중분은 분리
  - 쉽게: 새로 생성·소비한 토큰(캐시로 재사용 안 된 부분)만 일반 작업 6k·복합 추론 노드 8k 이하로 잡고, 캐시에서 값싸게 가져온 토큰은 따로 센다. 그래야 큰 컨텍스트를 캐시하는 좋은 전략이 "토큰 많이 썼다"고 벌점받지 않는다. *prompt caching = 반복되는 긴 입력을 캐시해 두고 정상가의 ~10%로 재사용.*
  > 보정(2026-06-24, 등급 척도 캘리브레이션 round-03): 구 `≤6k`(단일 헤드라인) → 신 `≤6k 일반 / ≤8k 복합 추론 노드`(★☆☆ 합격 하한=8k) — 아래 tier(★)를 정식 ★ rubric으로 승격하며 합격 하한을 복합 노드 캡(8k)에 맞춤. ★ 급간은 ## 등급 척도 참조.
- **(보조→정식 ★ rubric) 작업 난이도별 토큰 tier** — ★★★ ≤4k(단일 작업) / ★★☆ 4k~6k(일반) / ★☆☆ 6k~8k(복합 추론 허용)
  - 쉽게: 작업이 어려우면 토큰을 더 써도 되게 난이도별로 등급을 둔다. 복합 추론 노드는 캡을 완화. (round-03에서 ## 등급 척도로 정식화 — 캐시 적중률 ≥85% 조건 하.)
- **(보조) 캐시 적중률 (cache hit-rate)** — 신규 토큰 `≤6k` 합격이 적중률에 좌우되므로 측정 정직성 차원에서 함께 노출
...
기존 신규-토큰 tier(`★★★ ≤4k / ★★☆ 4k~6k / ★☆☆ 6k~8k`)를 정식화한다. 헤드라인 `신규 토큰 캡 + 캐시 토큰 분리집계`에서 분리집계는 측정 게이트(pass/fail)이고, 별점은 gradable한 **신규 토큰 양(cache-miss+output)**에 매긴다 (gradable이라 QA 성립).

**조건 (2-index):** `조건: 캐시 적중률 ≥ 85% 고정(A/B의 caching On, 대표 노드셋)` — 적중률이 신규 토큰 양을 좌우하므로(QA 본문 silent cap), 동일 적중률에서만 두 설계 대안의 신규 토큰을 비교해야 공정. 적중률 미달이면 별점 무효(게이트). 캐시·신규 미분리집계도 게이트 위반.

| 등급 | 구간 — 주 KPI(main 축): 작업당 신규 토큰(cache-miss+output) | 필드 근거 (경계 이유 + URL) |
|---|---|---|
| ★★★ (상) | ≤ 4k (단일/저복잡 노드) | Claude Code 92% 적중 시 effective 신규분이 list의 ≈1/7 — 고적중 단일작업 현실 최상위, 이론 천장에 −margin ([Requesty](https://www.requesty.ai/coding-agent-economy): 92% cache, $0.05/call · [Anthropic caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching): cache read=base 10%) |
| ★★☆ (중) | 4k ~ 6k (일반 파이프라인 노드) | 플랫폼 평균 적중 86% 대역 일반 에이전트 신규 토큰 현실 우수 구간 ([Requesty](https://www.requesty.ai/coding-agent-economy) platform avg 86%) |
| ★☆☆ (하) | 6k ~ 8k (복합 추론 노드 — 합격 최소선=KPI 하한) | 빌드로그·config diff 등 대입력 복합 노드의 합격 진입선. 적중률 변동·복합추론 margin 흡수 |
...
**무엇을 했나**
- **기존 tier 정식화**: 보조였던 신규-토큰 tier(`★★★ ≤4k / ★★☆ 4k~6k / ★☆☆ 6k~8k`)를 정식 `## 등급 척도`로 승격. main 급간 축 = `작업당 신규 토큰(cache-miss+output)`. (역배치 아님 — 신규 토큰은 낮을수록 ★ 높음.)
