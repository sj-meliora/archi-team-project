...
> 이 QA는 "장애 단위의 **복구**(죽은 걸 되살려 이어가기)"만 다룬다 — 한 장애가 다른 단위로 번지지 않게 막는 "**격리**(blast-radius 차단)"는 QA-08에서 따로 본다(서로 겹치지 않게 분리).
...
- QA-08과 경계 중복(둘 다 fault) — QA-02=복구 / QA-08=격리 명문화 필요.
