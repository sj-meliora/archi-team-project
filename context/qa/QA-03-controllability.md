...
updates:
  ...
  - date: 2026-06-24
    ...
    reason: "② 위반0 → ②-1 권한외 차단율(룰 게이트·현 조건 닫힘) + ②-2 적대적 위반0(NQA-A 채택 OI-8 의존 open-issue) 2단 분리 + '동반 닫힘' 조건부 정정 (contention 반영, 자세히 → ## 변경 이력)"
---
...
> 비대칭 사유(QA-09 대비): "절대값 KPI는 대리/대조로 시연한다" 원칙은 QA-09와 동일하나, **QA-09는 ②(`pass^k`)가 헤드라인이라 헤드라인을 교체**했고 **QA-03은 ②가 처음부터 보조라 헤드라인 ①은 유지하고 ②의 표현만 통일**한다(KPI 지위 차이로 적용 지점만 갈림). 따라서 ②는 보조 KPI로서 ②-1(현 조건 닫힘)·②-2(open-issue)로 2단 분리한다.
...
> 폐기: 旧 `허용 범위 외 액션 통과 = 0` 단독 — "없음"을 시연할 수 없는 절대값 + 풀세트 구축 전엔 산출 불가. → ②-1 차단율(현 조건 닫힘)/②-2 acceptance(NQA-A 채택 의존 open-issue)로 2단 분리(round-02 contention).
...
| ②-2 적대적 위반0 (acceptance) | **DP-0003 1안** + **eval/검증 서브시스템(NQA-A 공유 red-team 하네스, OI-7)** | 보조 모델(발표 서사): 적대적 eval 풀세트(OWASP LLM Top-10 매핑·injection·권한상승)로 통과/차단 — **실제 세트 구축은 NQA-A와 공유**, 본 라운드 미실행. **②-2·NQA-A 주 KPI는 NQA-A 채택(OI-8) 시 동일 하네스로 동반 닫힘**(무조건 동반 닫힘 아님 — 채택 의존 open-issue). 세트 커버리지 = 위반0의 신뢰 상한(미상상 우회 미검출, OWASP 매핑 log). |
...
- **R1 — "동반 닫힘"의 조건부성 은폐**: ② `위반0`의 실측 전환이 NQA-A 정식 채택(OI-8·사람 결정)에 통째 의존하는데, counsel이 이를 "동반 닫힘"이라는 무조건처럼 읽히는 표현으로 낙관(round-02 counsel line 69).
- **R2 — 절대값 KPI 시연 불가 + QA-09 무브 미적용 비대칭**: `허용범위 외 통과=0`은 QA-09 `pass^k 절대값`과 동형의 "없음을 시연할 수 없는" 절대값인데, 같은 ASR 세트에서 QA-09엔 "절대값→대리/대조" 무브를 쓰고 QA-03 ②엔 안 쓴 비대칭.

**무엇을 바꿨나 (반영)**
- **② 2단 분리**(QA-09 ②-1/②-2 아키타입 이식): ② `허용범위 외 통과=0` →
  ...
  - **②-2 적대적 위반0 acceptance** = OWASP LLM Top-10 매핑 적대적 풀세트 통과=0. **NQA-A 공유 하네스 풀커버리지 의존 → OI-8 채택 시 닫히는 추적 open-issue로 명시 강등**(커버리지 silent cap·OWASP 매핑 log 유지).
- **"동반 닫힘" 조건부 정정**: 무조건 "동반 닫힘" → "②-2·NQA-A는 OI-8 채택 시 동일 하네스로 동반 닫힘; ②-1은 OI-8 무관 선닫힘"으로 검증 전략·KPI에 조건 명시.
- **헤드라인 불변 명시**: 주 KPI(헤드라인·PoC 대상)는 ① 유지(시연 가능 — 교체 불요). QA-09와의 비대칭 사유(② 지위=헤드라인 vs 보조)를 KPI 섹션에 1줄 명시.
- 짝 시나리오 `QAS-03`의 Response·Measure를 ②-1/②-2 2단으로 동기화.

**남은 일 (이 라운드에서 미반영)**
- **②-2 ↔ NQA-A 하네스 채택(OI-8)**: 적대적 풀커버리지 위반0 실측은 NQA-A 정식 채택(사람 결정) 동반 — `open-issues.md` OI-8에 교차 의존 등록(QA-09 ②-2 ↔ NQA-B와 동일 구조).
