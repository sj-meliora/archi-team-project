...
> 이 QA는 "제어성(중단·권한 집행)"만 다룬다 — 비밀관리·공급망 서명·injection 방어·감사를 포괄하는 **보안 전반은 C-03(보안·안전 제약)**에서 따로 본다(Controllability ⊂ Security, 제어성은 안전의 수단). 경계는 C-03과 양방향 cross-link. *(보안 전반은 2026-06-26 QA-06 → 제약 C-03 이관, OI-10.)*
...
- **②-1 권한외 액션 차단율 — 주입한 hard-invalid 시도 → 실행 전 차단/안전정지 latency 분포** (룰 체커·admission 게이트, C-03/하네스 풀세트 불요)
  - 쉽게: 명백히 권한 밖인 행동(허용 안 된 도구 호출·범위 밖 배포·스키마 위반 = **hard-invalid**, 룰로 즉시 판정 가능)을 일부러 주입했을 때, 실행 전(admission) 게이트가 이를 차단하는 비율과 **차단/안전정지까지 걸린 시간 분포**를 잰다. "위반이 없음"(시연 불가)이 아니라 "주입한 위반을 막는 메커니즘이 작동함"(시연 가능)을 보이는 게 핵심 — ①의 `취소→ack/정지 분포`와 같은 "주입→차단/정지 분포" 아키타입으로 통일한다. *hard-invalid = allowlist·스키마·범위로 룰 판정되는 명백한 위반, admission = 토큰 쓰기 전 실행 전 차단.* **C-03 하네스 구축과 무관하게 현 조건에서 산출되는 1차 게이트.**
- **②-2 적대적 위반 통과 = 0 (acceptance)** — 적대적 eval 풀세트(OWASP LLM Top-10 매핑) 기준, 세트 커버리지가 신뢰 상한 `(open-issue — C-03 공유 red-team 하네스 구축 의존, OI-7)`
  - 쉽게: injection·권한상승처럼 **룰로 미리 표현 안 되는 미상상·우회 공격**까지 포함한 적대적 풀세트에서 통과가 단 한 건도 없어야 한다. 이 "0건"은 *세트가 상상한 공격에 대해서만* 0건이라 커버리지(OWASP LLM Top-10 매핑)를 함께 로그해 과대주장을 막는다. 이 풀커버리지 실측은 **공유 red-team 하네스(C-03)** 구축에 묶이므로, **그 하네스(eval 서브시스템 DP, OI-7)가 구축되면 닫히는 추적되는 open-issue**다 — 무조건 "동반 닫힘"이 아니라 *구축되면* 닫힘. *적대적 eval = 권한 우회·injection을 유도하는 red-team 시나리오 세트.*
...
> 폐기: 旧 `허용 범위 외 액션 통과 = 0` 단독 — "없음"을 시연할 수 없는 절대값 + 풀세트 구축 전엔 산출 불가. → ②-1 차단율(현 조건 닫힘)/②-2 acceptance(C-03 하네스 구축 의존 open-issue)로 2단 분리(round-02 contention).
...
| **위반 0건 = 적대적 eval 통과율** | "허용범위 외 0건"은 구호가 아니라 red-team eval 세트 통과율로만 검증 가능 — 측정 수단을 KPI 정의에 묶음 (C-03 red-team PoC와 공유) | counsel §4 제어·안전 (별도 URL 없음 — eval 세트는 우리가 구축) |
...
### 2026-06-26 — QA-06 → C-03 이관에 따른 cross-ref redirect (OI-10)
출처: 팀 결정(OI-10) — QA-06 Security/Safety를 제약 `C-03`으로 이관.
- 라이브 섹션(정의 altitude·측정 ②-1/②-2·근거·검증 전략·캘리브레이션 노트)의 "QA-06" 참조를 **C-03**으로 redirect. ②-2 의존을 "QA-06 정식 채택(OI-8) 의존" → **"C-03 공유 red-team 하네스 구축(OI-7) 의존"** 으로 재명명(채택은 C-03 이관으로 완료, 잔존 의존은 하네스 구축뿐).
- Controllability ⊂ Security 경계 cross-link 상대를 QA-06 → C-03으로 변경. 위 변경 이력(round-01~04)의 과거 QA-06 서술은 당시 번호로 보존.
