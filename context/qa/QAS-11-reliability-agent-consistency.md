...
> category: QAS | refines: QA-11 (Reliability-Agent) | updated: 2026-06-24 (round-01 contention 반영) (round-03 ★ 급간 동기화)
...
| **응답 측정 (Measure)** | **(주) 캐시 On/Off 일관성 갭 Δ** 대조 시연(명제 "캐시는 일관성 입증 아님); (보조) 순수 추론 pass^k(k=5; ★ 급간 ≥25/40/60%); ②-1 대리 유효-결정률 **≥95%**(룰 게이트); ②-2 정밀 유효-결정률(QA-07 의존, open-issue); 재실행 분산 **H_norm ≤0.2** |

## 비고
- 설계 연결: DP-0005(2안 공유 캐시 = memoization — **측정 시 캐시 우회 모드 필수**, 캐시는 비용 지표이지 일관성 입증 아님).
- 수치(pass^k 25/40/60·95%·0.2)는 예시값 — 합격선은 캐시 우회 반복시행으로 확정. 상세는 QA-11 본문. (★ 급간은 QA-11 등급 척도)
