...
> category: QAS | refines: QA-08 (Reliability-Workflow) | updated: 2026-06-24 (round-03 ★ 급간 동기화)
...
| **응답 측정 (Measure)** | 타 WF 실행 중단 **≤1%**, latency 증가 ≤25%(★☆☆; 10%=★★☆), 단일 WF의 토큰/rate-limit 쿼터 침범 **0건**, 공유 캐시 오염 전파 **0건** |

## 비고
- 설계 연결: DP-0004(A5/A8 invocation·단계 bulkhead), DP-0005(1안 로컬 캐시=오염 격리 / 2안 공유 캐시=무효화·읽기전용 계층화 필요).
- 수치(1%·10%·0건)는 합격선 — ①②는 기존 건강 KPI 유지, ③④ 임계는 실환경 측정으로 확정. 상세는 QA-08 본문. (★ 급간은 QA-08 등급 척도; round-04 ★★★ ≤5% margin 근거화·noisy-neighbor apples silent cap·쿼터 0건 Constraint 유지 — 급간 수치 불변)
