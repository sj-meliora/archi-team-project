...
## [ ] OI-13 DP-06 지식 베이스 — RAG vs LLM 위키 택일 미확정
- **맥락(2026-08-10)**: DP-06(Agent 지식 베이스) 신설 — 자율 이슈처리(불량 분석·회귀)의 전제인 지식 공급 계층이 어떤 DP에도 없던 공백을 닫음. 슬라이드 본편 3장(필요성 / 공통 조회 계층 구조 / 대안 비교) + Appendix 1장(Cerebras) 구성.
- **경위(같은 날 v1→v2→v3)**: v1 "RAG vs LLM Wiki" 형태 비교 → v2 디스커션: 인덱싱·조회 계층은 공통 전제(채택 패턴 = Cerebras Knowledge), 결정 축을 부트스트랩 전략으로 전환 → **v3 발표 용어 정비: 대안을 청중이 아는 이름 2개(1안 RAG / 2안 LLM 위키)로 단순화**, v2의 3안(증류 시딩)은 1안 강화 tactic으로, LLM-as-a-judge(QA-07 기법 재사용)를 2안 강화 tactic으로 재배치. 조회 계층 상세는 `_backlog` BL-4.
- **미확정**: ① 1안(RAG+증류 시딩 — 권고) vs 2안(LLM 위키+judge) 택일 — 드라이버 Q1(기존 소스에 건질 지식 실재 여부)·Q2(초기 신뢰 형성 binding 여부)를 팀/PM이 판정, DP 디스커션 회부. ② `◯` 해제 — 1안 초기 ★★★◯(시딩 증류 품질 HITL 표본 감사 PoC), 2안 정상 ★★★◯(judge κ≥0.8 선검증, QA-07 조건 공유). ③ ~~도안 SVG 미작성~~ → **작성 완료(2026-08-10, `dp/DP-06/diagrams/` — 폴더 승격)**; 선택안 확정 시 선택안 강조 버전으로 갱신만 남음. ④ BL-4(조회 계층 상세 설계) 승격 여부. ⑤ A4 수렴 노트(1안 강화형 → 2안 수렴 경로)의 정식 대안 승격 여부. ⑥ **KB-DQ(v3.1 신설 — ISO/IEC 25012 앵커 지식 데이터 품질 기준: 커버리지·신선도·축적 속도)의 QA 승격 여부** — 현재 DP-06-로컬 보조 매트릭스(기존 QA-01 행 대체), 전사 QA(비-ASR)로 올릴지 팀 디스커션 + 예시 KPI(context recall·stale-hit율·time-to-knowledge) PoC 확정.
- **남은 작업**: 선택안 확정 시 — module-view Repository 레이어에 `Knowledge Base`(단일 조회 계층 + 증류·검증 파이프라인) 반영, QA-07/05/01 `related-dp`에 DP-06 역방향 링크 추가, eval/검증 서브시스템 DP(OI-7)와 judge 하네스 공유(2안 tactic·R-3 방어) 정합 확인.
