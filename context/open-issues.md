...
- **미확정**: ① 1안(RAG+증류 시딩 — 권고) vs 2안(LLM 위키+judge) 택일 — 드라이버 Q1(기존 소스에 건질 지식 실재 여부)·Q2(초기 신뢰 형성 binding 여부)를 팀/PM이 판정, DP 디스커션 회부. ② `◯` 해제 — 1안 초기 ★★★◯(시딩 증류 품질 HITL 표본 감사 PoC), 2안 정상 ★★★◯(judge κ≥0.8 선검증, QA-07 조건 공유). ③ 도안 SVG 미작성(선택안 확정 후 `diagrams/` 승격). ④ BL-4(조회 계층 상세 설계) 승격 여부. ⑤ A4 수렴 노트(1안 강화형 → 2안 수렴 경로)의 정식 대안 승격 여부.
