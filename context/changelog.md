...
## 2026-08-13 — DP-06 v4: 거버넌스 재프레이밍 — LLM 위키 기반 채택, 대안 = 차등 게이팅 vs 정본+런타임 공유
- 변경: **결정 축 전환** — "지식을 무엇으로 채우나(1안 RAG vs 2안 LLM 위키)" → **"위키 갱신을 어떻게 거버넌스하나"**. 팀 방향: **LLM 위키를 기반(base)으로 채택**(v3 A4 수렴 노트 채택 — 증류 시딩 산출과 위키 문서는 동형이라 형태 택일 종료). 위키 = 기존 소스(**IMS(Jira)·Confluence·Slack·빌드 로그**) **증류 md 문서로 구축**(콜드스타트를 증류 시딩으로 닫음, git 관리 → PR 단위 갱신) + 운영 축적. 공통 전제 3 신설: **정본(설계·정책) 지식의 사람(시니어·아키텍트) PR 게이트는 양안 공통** — 대안이 갈리는 건 비정본 지식(팁·현황·최신 정보)의 흐름뿐.
- 변경(새 대안): **1안 지식 등급별 차등 게이팅** — 위키를 정본/운영 티어로 구분, 정본=사람 PR 게이팅(CODEOWNERS), 운영=**LLM-as-a-judge 검증 auto-merge**(κ≥0.8, QA-07 하네스 공유) + 승격·단방향 참조 규율. 원형 = Wikipedia 문서 보호 정책. / **2안 엄격 정본 + 런타임 공유** — 위키는 전량 사람 PR 게이팅(순도 100%), Agent 생성 최신 정보는 위키 밖 **런타임 공유 계층**(Blackboard 패턴)에서 무게이트 즉시 게시·구독(TTL·provenance 표식, 미검증 근거 비가역 액션 금지=C-03), 승격 큐→증류→사람 게이트→정본.
- 변경(평가): **KB-DQ-4 정합성(25012 Consistency) 신설**(거버넌스 축의 핵심 변별 기준 — 모순 검출률·정본 위반 등재 건수). 매트릭스 재작성: 1안 = 축적 속도·커버리지·Efficiency 우위 / 2안 = 정본 정합성·신선도(런타임 즉시) 우위. ATAM 재구성(R-1 오분류(1안)·R-2 승격 병목(2안)·R-4 런타임 무검증 전파(2안), NR-3 계층 직교 → 1안+런타임 계층 3계층 수렴 경로). 권고 = **1안**(드라이버 Q1: 시니어 리뷰 대역폭이 전량 갱신을 감당 못 함 — 사람 병목 제거가 시스템 목적).
- 변경(도안): `1an-rag.svg`·`2an-llm-wiki.svg` 삭제 → **`1an-tiered-gating.svg`·`2an-canonical-runtime.svg` 신규**(두 게이트 차등 / 런타임 계층+승격 큐 — 위험 표기 포함). 슬라이드 2 mermaid를 "갱신 게이트 = 결정 포인트" 구조로 재작성.
- 사유: 팀 디스커션(2026-08-13) — 위키 기반은 합의됐고, 실제 긴장은 "정합성 엄격 지식의 사람 검증"과 "사람 병목 없는 축적·공유"의 양립 — 게이트를 어디에 긋나가 남은 결정.
- 영향 ID: DP-06(v4), diagrams(구 2 삭제·신규 2), open-issues(OI-13 재프레이밍), _backlog(BL-4 문구), INDEX, changelog.

## 2026-08-13 — DP-06 v4.1: repo QA 12종 전수 스크리닝 (A3′ 신설)
- 변경: repo 정의 QA 전체(QA-01~13, QA-06 공석) 기준으로 두 대안 접점·변별 전수 판정 — **A3′ 신설**. v3.1의 "물리는 ASR은 QA-07·QA-05뿐"은 '무엇으로 채우나' 축 기준이었음을 확인, **거버넌스 축에서는 QA-01(갱신 처리량 확장 — 1안 ★★★/2안 ★☆☆)·QA-03(HITL 게이트 원칙 — 1안 ★★☆◯/2안 ★★★)·QA-04(결정 재구성 — 1안 ★★★/2안 ★★☆)가 추가 변별**. 비-ASR 참고 판정: QA-08(blast-radius — 2안 우세)·QA-11(결정 일관성)·QA-12(유지보수 표면)·QA-13(사람 공수 재도입 — 이상 1안 우세). 비변별 = QA-02·09·10(QA-09는 QA-07에 흡수 — 이중 계상 금지).
- 변경(결론): 합산 관찰 — ASR 편들기: 1안 = QA-01·04·05·07(운영), 2안 = QA-03·07(정본). **권고(1안) 유지 + QA-03 보상 통제(judge 에스컬레이션·오분류 감사·갱신 rate cap)를 이행 조건으로 명시**(1안 ★◯ 해제 조건과 동일 집합). 헤더 drives에 보조 접점 표기, 슬라이드 4 앵커에 A3′ 참조 추가.
